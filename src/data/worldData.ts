...
export type GalleryCategory = "houses" | "museums" | "market";
...
export const GALLERY_ITEMS: GalleryItem[] = [
  ...HOUSES.map((house) => ({
    ...
    title: house.name,
    subtitle: `Résidence • niveau ${house.levelRequired === 9999 ? "Hime-sama" : house.levelRequired}`,
    image: house.image,
  })),
  ...MUSEUMS.map((museum) => ({
    id: `museum-${museum.houseId}`,
    category: "museums" as const,
    title: `🏛️ ${museum.name}`,
    subtitle: `Musée lié à ${HOUSE_BY_ID[museum.houseId].name}`,
    image: museum.image,
  })),
  ...Object.entries(MARKET_STAGE_IMAGES).map(([stage, image]) => ({
    id: `market-stage-${stage}`,
    category: "market" as const,
    title: `🏘️ Marché • étape ${stage}`,
    subtitle:
      Number(stage) === 0
        ? "Marché en ruines"
  ...
  ...MARKET_BUILDINGS.filter((building) => building.interiorImage).map(
    (building) => ({
      id: `market-${building.id}`,
      category: "market" as const,
      title: `${building.emoji} ${building.name}`,
      subtitle: `${building.merchant.name} • ${building.merchant.title}`,
      image: building.interiorImage!,
    }),
  ),
];
